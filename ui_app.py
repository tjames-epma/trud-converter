...
def get_ampp_data(zip_obj, file_pattern):
    ...
    with zip_obj.open(matches[0]) as f:
        ...
        for record in root.findall(".//{*}AMPP"):
            ...
            # Ensure ABBREVNM exists even if XML tag is missing
            if "ABBREVNM" not in entry:
                entry["ABBREVNM"] = ""
            rows.append(entry)
...
def get_gtin_mapping(zip_obj):
    ...
    with zip_obj.open(xml_files[0]) as f:
        ...
        for ampp_block in root.findall(".//{*}AMPP"):
            # Check for AMPPID or APPID (Double-Look)
            id_elem = ampp_block.find("{*}AMPPID") or ampp_block.find("{*}APPID")
            ...
            for gtin_data in ampp_block.findall(".//{*}GTINDATA"):
                ...
                if gtin_elem is not None and row_id:
                    rows.append({'JOIN_ID': row_id, 'GTIN': gtin_elem.text})
        return pd.DataFrame(rows)
...
with st.sidebar:
    ...
    lookup_id = st.text_input("Enter APPID/AMPPID to find GTIN")
    st.divider()
    st.caption("v1.6 | Stable Build")
...
with col2:
    ...
    if uploaded_file:
        ...
        if date_match:
            ...
            st.warning(f"📅 **Date Identified:** {week_num}")
    else:
            ...
            try:
                with zipfile.ZipFile(uploaded_file, 'r') as outer_zip:
                    ...
                    if not gtin_zip_list:
                        ...
                        # Improved Merge Logic (Double-Look)
                        st.write("Merging records...")
                        # 1. Try joining on APPID
                        final_df = pd.merge(df_ampp, df_gtin, left_on='APPID', right_on='JOIN_ID', how='left')
                        
                        # 2. If APPID merge yielded no GTINs, try AMPPID
